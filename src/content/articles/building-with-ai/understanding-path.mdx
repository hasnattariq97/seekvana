...
lessonNumber: "03.09"
faqs:
  - q: "What exactly is an environment variable?"
  ...
  - q: "Why does reinstalling fix a PATH problem?"
    a: "When you reinstall a tool and choose the \"Add to PATH\" option, the installer writes the tool's folder location into your system's PATH environment variable. The tool was never the problem, the missing registration was. Reinstalling with the right option checked just adds the address to the list."
  - q: "I ticked the checkbox but the terminal still says command not found, why?"
    a: "Your terminal reads PATH when it first opens. If your terminal was already open when you reinstalled, it loaded the old PATH and doesn't know about the new entry. Close the terminal completely, not just the tab, the whole window, and open it again. Then try the command. This fixes the problem 90% of the time."
  - q: "Does PATH affect my code, or just the terminal?"
    a: "PATH only affects the terminal, it determines which commands the shell can find and run. Your code doesn't use PATH directly. However, when you run your code from the terminal (like `python my_script.py`), the terminal uses PATH to find the Python executable. So if Python isn't in PATH, you can't run your code from the terminal, but PATH itself isn't part of your program."
---
...
Here's what actually happened when you installed Python and got "command not found": the Python executable was placed on your hard drive, but its folder wasn't added to PATH. That phrase is one of a handful the terminal reuses constantly, and [learning to read terminal errors](/library/building-with-ai/reading-error-messages) tells you which ones mean a PATH problem and which mean something else entirely.
...
<Note>
 Done? You've completed Lesson 03.09.
</Note>
