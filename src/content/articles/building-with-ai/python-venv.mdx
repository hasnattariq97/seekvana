...
{/* IMAGE PROMPT: Editorial flat illustration of two separate open folders side by side, each containing its own small Python logo and stack of package icons, warm cream background, clay accent (#C9633F) outlining each folder, clean and minimal, no text in image, Seekvana palette: #FAF8F3, #C9633F | FILENAME: cover.webp | DIMENSIONS: 1200x630 */}
{/* IMAGE PROMPT: Split terminal window illustration, left half showing a plain $ prompt labeled "before", right half showing a (venv) $ prompt labeled "after", warm cream background, clay accent on the (venv) text, minimal and clean, Seekvana palette: #FAF8F3, #C9633F | FILENAME: venv-prompt-before-after.webp | DIMENSIONS: 800x450 */}
...
<ArticleImage
 src="/images/articles/building-with-ai/python-venv/venv-prompt-before-after.webp"
 alt="Terminal prompt shown plain before activation and prefixed with (venv) after activation"
 caption="The (venv) prefix is the only visual signal telling you which Python your commands are actually using."
/>
