...
faqs:
  ...
  - q: "Do file paths work the same on Windows and Mac?"
    a: "The concept is identical, both use a tree of folders with a root at the top. The differences are cosmetic: Mac and Linux use a forward slash (`/`) as the separator and start from `/` (root), while Windows uses a backslash (`\\`) and starts from a drive letter like `C:\\`. Most modern Windows tools (including Git Bash and PowerShell) also accept forward slashes, so the practical difference is smaller than it looks."
  - q: "How do I find my absolute path if I don't know it?"
    a: "Run `pwd` in your terminal, it prints the full absolute path to your current folder. If you need the absolute path to a specific file inside that folder, run `pwd` to get the base, then add the filename. On Mac, you can also right-click any file in Finder, hold the Option key, and choose \"Copy as Pathname\" to get its absolute path without touching the terminal."
---
