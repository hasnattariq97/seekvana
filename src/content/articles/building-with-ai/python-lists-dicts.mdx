...
- Tool definitions (what tools an [AI agent](/glossary/agent) can use)
- Search results (a list of documents retrieved for [RAG](/glossary/rag))
...
Two messages. Two dictionaries. One list. This is the exact structure passed to `client.messages.create()` in every Claude API call you'll ever write or read. Later in this module, you'll see this same [database table shape](/library/building-with-ai/database-tables-for-agents) show up again, just stored as rows instead of a Python list.
...
That `"content"` string is the one part of this whole structure you fully control. [Writing better AI prompts](/library/building-with-ai/prompting-and-code) is really just learning to write that string well.
