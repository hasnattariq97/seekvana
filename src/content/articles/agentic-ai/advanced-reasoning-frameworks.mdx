...
faqs:
  ...
  - q: "Do frontier reasoning models make these frameworks obsolete?"
    a: "Partly. Models with built-in extended thinking already run something like an internal search before answering, so a hand-built Tree of Thoughts on top of one is often redundant. These frameworks still earn their cost when you need to see the reasoning, control the search yourself, or you're pairing a cheap model with an expensive judge, and the same quality-vs-cost tradeoff applies to setting the model's own thinking budget in the first place."
---
...
<FAQ>
  ...
  <FAQItem question="Do frontier reasoning models make these frameworks obsolete?">
    Partly. Models with built-in extended thinking already run something like an internal search before answering, so a hand-built Tree of Thoughts on top of one is often redundant. These frameworks still earn their cost when you need to see the reasoning, control the search yourself, or you're pairing a cheap model with an expensive judge, and the same [quality-vs-cost tradeoff](/library/agentic-ai/reasoning-models-and-test-time-compute) applies to setting the model's own thinking budget in the first place.
  </FAQItem>
</FAQ>
