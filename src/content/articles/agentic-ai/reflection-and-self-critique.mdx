...
Both patterns assume the reasoning skills of chain-of-thought and the tool-calling of a [reasoning-before-acting loop](/library/agentic-ai/chain-of-thought-for-agents) are already in place; reflection is what happens after the agent has already acted and gotten a result back.
...
Add a reflection step when two things are both true: the task is genuinely failure-prone (multi-step code, a flaky API call, a spec with edge cases), and you have a real signal to reflect on once it fails (a test suite, a schema validator, an execution trace).
