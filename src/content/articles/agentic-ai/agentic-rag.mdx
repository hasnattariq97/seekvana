...
That "retrieve only when needed" instinct is a specific case of a broader choice: whether an agent pre-loads context up front or fetches it [just-in-time](/library/agentic-ai/just-in-time-vs-preloaded-context), and the same latency-cost-relevance tradeoff applies whether the thing being fetched is a document chunk or a tool schema.
