...
An unsafe server is one where a tool's real behavior doesn't match its name, where it asks for more access than its stated purpose needs, or where you granted broad access once and never looked again. **Tool poisoning** (a tool that looks harmless, like a calculator, but is written to do something else entirely) and **rug-pulls** are both variations on the same failure: you trusted the label instead of checking the substance. Everything in the checklist above exists to close that gap. Later in this module, [MCP Security and the Trust Boundary](/library/agentic-ai/mcp-security-and-the-trust-boundary) goes deeper into threat-modeling a server someone else built, including auditing one that's deliberately been made vulnerable.
