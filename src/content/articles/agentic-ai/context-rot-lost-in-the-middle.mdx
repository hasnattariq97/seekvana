...
The confusion is worth naming upfront, because it changes what you'd even try to fix. A stale rules file gets fixed by editing the file. Context rot gets fixed by managing what you put in [the context window](/library/large-language-models/what-is-a-context-window) in the first place, which is why it sets up the next several lessons in this module.
...
Compaction fixes context rot after it's already accumulated. A [just-in-time context strategy](/library/agentic-ai/just-in-time-vs-preloaded-context) prevents most of it from accumulating in the first place, by never pre-loading the data that would have rotted in the middle.
