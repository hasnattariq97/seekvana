...
| Reopening later | A small persistent "Cookie preferences" tab, rendered by `CookieBanner` itself once a decision has been made — see correction below (originally planned as a footer link; footer isn't rendered sitewide, so that wouldn't have been reachable from most pages) |
...
- No separate reopen-link component. **Correction (found while planning, 2026-07-18):** the spec originally planned a footer link to reopen preferences, but `Footer` (`src/components/layout/footer.tsx`) is only rendered on the homepage (`src/app/page.tsx`) — verified via repo-wide grep, no other page renders it. A footer-only link would be unreachable from every article, path, and library page, i.e. most of the site. Fixed by moving the reopen affordance into `CookieBanner` itself: once a decision exists (`initialConsent` is non-null on mount, or the visitor just made a choice), the component renders a small persistent tab — bottom-left corner, unobtrusive, similar in spirit to the existing bottom-right orange "Feedback" tab (`SideFeedback`) already on every page — showing something like "🍪 Cookie preferences". Clicking it switches the same component's internal state back to the expanded Manage view, pre-filled with the current stored choice. Because `CookieBanner` is rendered from `ConsentGate` in the root layout (sitewide), this is reachable from every page with no cross-component state-sharing needed at all — simpler than the original footer-link design, not just a fallback for it.
...
6. **After 6 months, or visitor clicks the persistent "Cookie preferences" tab**: banner reappears (expired cookie → `null` state) or reopens in its expanded Manage view (tab-click path, available on every page) pre-filled with the current toggle state, so the visitor can change their mind — satisfies the "withdraw consent anytime" requirement.
...
- No unit tests planned for `CookieBanner`/`ConsentGate` themselves — this repo has no existing React component test harness (only Vitest for pure logic/server actions), and introducing one would be new infrastructure beyond this feature's scope. Verified manually instead:
  ...
  - Persistent "Cookie preferences" tab: visible on an article page and a path page (not just the homepage), reopens banner in expanded Manage state with current toggle values reflected correctly.
