...
**Modified files:**
- `src/app/layout.tsx` — call `getConsent()` (server-side, in the root layout Server Component), pass the result to `<CookieBanner initialConsent={consent} />`. Wrap the existing `gtag`/`adsbygoogle` `<script>` tags in conditionals: GA loads only if `consent?.analytics === true`; AdSense loads only if `consent?.advertising === true`.
